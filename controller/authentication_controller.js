  ...
  try {
    ...
    if (isValidUser) {
      ...
      if (!getResult) {
        ...
        let createResult = await userModel
          .createUser(new userModel(newUser))
          .catch(err => console.log(err));
        return res.json({
          success: true,
          displayUI: true,
        ...
        return res.json({
          success: false,
          msg: "Email, tên đăng nhập hoặc số điện thoại đã tồn tại"
        });
      }
    } else {
      return res.json({
...
module.exports.validate = (req, res, next) => {
  return userModel
    ...
    .then(users => {
      ...
      let userUsernames = users.map(user => user.username);
      let userPhoneNumbers = users.map(user => user.phoneNumber);
      let existEmail = userMails.includes(req.body.email + "");
      let existUsername = userUsernames.includes(req.body.username + "");
      let existPhoneNumber = userPhoneNumbers.includes(
